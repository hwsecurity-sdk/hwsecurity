...
package de.cotech.hw.openpgp.internal.ssh;
...
public class SshUtil {
    ...
    public static final String OPENSSH_CERT_SUFFIX = "-cert-v01@openssh.com";

    @NonNull
    public static String encodeKeyBlobAsKeyString(String algorithmName, byte[] sshEncodedData) {
        String keyBlob = Base64.toBase64String(sshEncodedData);
        return algorithmName + " " + keyBlob;
    }

    @NonNull
    public static String stripAlgorithmCertSuffix(@NonNull String algorithmName) {
    ...
    public static String encodeEcPublicKeyString(ASN1ObjectIdentifier oid, ECPublicKey publicKey)
            throws NoSuchAlgorithmException {
        String curveName = getCurveName(oid);
        byte[] sshEncodedData = encodeEcPublicKey(curveName, publicKey);
        return encodeKeyBlobAsKeyString("ecdsa-sha2-" + curveName, sshEncodedData);
    }

    public static byte[] encodeEcPublicKey(ASN1ObjectIdentifier oid, ECPublicKey publicKey)
            throws NoSuchAlgorithmException {
        return encodeEcPublicKey(getCurveName(oid), encodeEcQ(publicKey));
    }

    private static byte[] encodeEcPublicKey(String curveName, ECPublicKey publicKey) {
        return encodeEcPublicKey(curveName, encodeEcQ(publicKey));
    }
    ...
    public static String encodeRsaPublicKeyString(RSAPublicKey publicKey) {
        byte[] sshEncodedData = encodeRsaPublicKey(publicKey);
        String keyBlob = Base64.toBase64String(sshEncodedData);
        return "ssh-rsa " + keyBlob;
    }

    public static byte[] encodeRsaPublicKey(RSAPublicKey publicKey) {
        SshEncodedData sshEncodedData = new SshEncodedData();
    ...
    public static byte[] encodeSshSignature(String sshAlgorithmName, byte[] signedChallenge)
    ...
    public static String getSignatureHashAlgorithmName(String sshAlgorithmName) throws NoSuchAlgorithmException {
    ...
    public static String getCurveName(ASN1ObjectIdentifier oid) throws NoSuchAlgorithmException {
