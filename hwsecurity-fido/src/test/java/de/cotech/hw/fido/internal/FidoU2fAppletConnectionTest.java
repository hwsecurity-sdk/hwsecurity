...
public class FidoU2fAppletConnectionTest {
    ...
    public void communicate_ping() throws Exception {
        transport.expect(PING_APDU, ResponseApdu.fromBytes(Hex.decodeHexOrFail("9000")));
        connection.communicateOrThrow(PING_APDU);
    }

    @Test
    public void communicate_ping_extended() throws Exception {
        FakeTransport transport = new FakeTransport();
        FidoU2fAppletConnection connection = FidoU2fAppletConnection.getInstanceForTransport(transport);
        transport.expect(PING_APDU_EXTENDED, ResponseApdu.fromBytes(Hex.decodeHexOrFail("9000")));
        transport.setExtendedLengthSupported(true);
